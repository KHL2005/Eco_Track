    ...
    <>
      <div className="overflow-x-auto rounded-xl border border-gray-100">
        <table className="w-full text-sm">
          ...
          <tbody>
            {documents.map((d, i) => (
              <tr key={d.documentId} className="border-t border-gray-50 hover:bg-bg/50 transition-colors">
                <td className="px-4 py-3 text-text-muted">{i + 1}</td>
                {showCompany && <td className="px-4 py-3 font-medium text-text">{d.industryName}</td>}
                <td className="px-4 py-3 text-text">
                  <span className="px-2 py-0.5 bg-primary/5 text-primary text-xs font-medium rounded-md">{d.docType}</span>
                </td>
                <td className="px-4 py-3 text-text-muted max-w-[200px] truncate">{d.description || '—'}</td>
                <td className="px-4 py-3 text-text-muted">{new Date(d.uploadedDate).toLocaleDateString()}</td>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {onApprove && d.verificationStatus === 'SUBMITTED' && (
                      <>
                        <button onClick={() => openModal(d.documentId, 'approve')} className="px-2.5 py-1 rounded-lg bg-accent/10 text-primary text-xs font-semibold hover:bg-accent/20 transition-colors cursor-pointer">Approve</button>
                        <button onClick={() => openModal(d.documentId, 'reject')} className="px-2.5 py-1 rounded-lg bg-error-light text-error text-xs font-semibold hover:bg-error/20 transition-colors cursor-pointer">Reject</button>
                      </>
                    )}
                    {onApprove && d.verificationStatus !== 'SUBMITTED' && (
                    ...
                    {onDelete && (
                      <button
                        onClick={() => openModal(d.documentId, 'delete')}
                        disabled={d.verificationStatus !== 'SUBMITTED'}
                        ...
                        title={d.verificationStatus !== 'SUBMITTED' ? 'Cannot delete' : 'Delete'}
                      >
      ...
      <ConfirmModal
        open={modal.open}
        title={`${actionLabels[modal.action] || 'Confirm'} Document`}
