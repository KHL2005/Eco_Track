...
export default function DashboardLayout({ children }) {
  ...
  useEffect(() => {
   ...
   const SidebarContent = ({ collapsed = false }) => (
    <div className="flex flex-col h-full">
      {/* Logo */}
      <div className={`flex items-center ${collapsed ? 'justify-center px-2' : 'gap-2 px-5'} py-5 border-b ${role === 'CITIZEN' ? 'border-white/10' : 'border-white/10'}`}>
        <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${role === 'CITIZEN' ? 'bg-white' : 'bg-leaf-400'}`}>
          <Leaf size={18} className={role === 'CITIZEN' ? 'text-emerald-600' : 'text-forest-900'} />
        </div>
        {!collapsed && <span className={`font-bold text-lg ${role === 'CITIZEN' ? 'text-[#7f1d1d]' : 'text-white'}`}>EcoTrack</span>}
      </div>

      {/* User info */}
      {!collapsed && (
        <div className={`px-5 py-4 border-b border-white/10`}>
          <div className={`text-xs mb-0.5 ${role === 'CITIZEN' ? 'text-[#7f1d1d]/80' : 'text-white/50'}`}>Signed in as</div>
          <div className={`text-sm font-medium truncate ${role === 'CITIZEN' ? 'text-[#7f1d1d]' : 'text-white'}`}>{user?.name}</div>
          <div className={`text-xs ${role === 'CITIZEN' ? 'text-[#7f1d1d]/70' : 'text-leaf-400'}`}>{ROLE_LABELS[role] || role}</div>
        </div>
      )}
          ...
          return (
            <Link
              ...
              className={`flex items-center ${collapsed ? 'justify-center' : 'gap-3'} px-3 py-2.5 rounded-xl text-sm font-medium transition-all
                ${active
                  ? role === 'CITIZEN' ? 'bg-white/15 text-[#7f1d1d]' : 'bg-white/15 text-white'
                  : role === 'CITIZEN' ? 'text-[#7f1d1d]/80 hover:bg-white/10 hover:text-[#7f1d1d]' : 'text-white/70 hover:bg-white/10 hover:text-white'
                }`}
            >
              <Icon size={18} />
  ...
  return (
    <div className={`flex h-screen overflow-hidden bg-earth-100`}>
      {/* Desktop sidebar */}
      <aside className={`hidden lg:flex flex-col ${role === 'CITIZEN' ? 'bg-[#004411]' : 'bg-forest-900'} flex-shrink-0 transition-all duration-200 ${sidebarCollapsed ? 'w-20' : 'w-64'}`}>
       ...
       {sidebarOpen && (
         <div className="fixed inset-0 z-50 lg:hidden">
           ...
           <aside className={`absolute left-0 top-0 bottom-0 w-64 ${role === 'CITIZEN' ? 'bg-[#004411]' : 'bg-forest-900'} z-10`}>
            <SidebarContent />
          </aside>
        </div>
      )}
